...
export class ContentAdapter implements Adapter<BridgeMessageMeta> {
	...
	public sendMessage: SendMessage<BridgeMessageMeta> = async (message) => {
		console.log("[ContentAdapter] Sending message:", message.type, message.path);
	...
	public onMessage: OnMessage<BridgeMessageMeta> = (callback): OffMessage | void => {
		const handler = (message: BridgeEnvelope<BridgeMessageMeta>) => {
			...
			console.log("[ContentAdapter] Received message:", message.payload?.type, message.payload?.path);
